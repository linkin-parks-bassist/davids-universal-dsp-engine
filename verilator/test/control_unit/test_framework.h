...
struct Test {
    const char* name;
    std::function<void(Vcontrol_unit*, VerilatedVcdC*)> fn;
};
...
#define TEST(name) \
    void name(Vcontrol_unit*, VerilatedVcdC*); \
    struct name##_registrar { \
        name##_registrar() { \
            get_tests().push_back({#name, name}); \
        } \
    } name##_registrar_instance; \
    void name(Vcontrol_unit* dut, VerilatedVcdC* tfp)
...
extern void settle(Vcontrol_unit* dut, VerilatedVcdC* tfp);
extern void tick(Vcontrol_unit* dut, VerilatedVcdC* tfp);
