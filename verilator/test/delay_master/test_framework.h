...
#include "Vdelay_master.h"
#include <verilated_vcd_c.h>

struct Test {
    const char* name;
    std::function<void(Vdelay_master*, VerilatedVcdC*)> fn;
};
...
#define TEST(name) \
    void name(Vdelay_master*, VerilatedVcdC*); \
    struct name##_registrar { \
        name##_registrar() { \
            get_tests().push_back({#name, name}); \
        } \
    } name##_registrar_instance; \
    void name(Vdelay_master* dut, VerilatedVcdC* tfp)
...
extern void settle(Vdelay_master* dut, VerilatedVcdC* tfp);
extern void tick(Vdelay_master* dut, VerilatedVcdC* tfp);
